...
- Verified resolved in v1.1.5: package/README/source manifest versions, README install text, changelog date, and test counts now align with 13 suites/87 tests.
...
- Testing gaps are clear: `npm test` passes 13 suites/87 tests and `npm run lint` passes, but there are no browser-loaded extension smoke tests, sync conflict/tombstone tests, provider-outage tests, a11y checks, or package-artifact verification.
